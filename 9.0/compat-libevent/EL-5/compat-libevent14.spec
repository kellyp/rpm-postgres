...
# We don't want conflicts with libevent-2.0 package, so remove some files here:
rm -f %{buildroot}%{_bindir}/event_rpcgen.py*
rm -f %{buildroot}%{_libdir}/libevent.a
rm -f %{buildroot}%{_libdir}/libevent.so
rm -f %{buildroot}%{_libdir}/libevent_core.a
rm -f %{buildroot}%{_libdir}/libevent_core.so
rm -f %{buildroot}%{_libdir}/libevent_extra.a
rm -f %{buildroot}%{_libdir}/libevent_extra.so
...
%doc README
%{_libdir}/libevent-*%{majorversion}.so.*
%{_libdir}/libevent_core-*%{majorversion}.so.*
%{_libdir}/libevent_extra-*%{majorversion}.so.*
...
%changelog
* Wed Aug 24 2011 Devrim GUNDUZ <devrim@gunduz.org> 1.4.13-1
- Update to 1.4.13
- Rename package to compat-libevent14
